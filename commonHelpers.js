...
`}function q(o){return o.map(M).join("")}function g(o){const t=q(o);p.insertAdjacentElementHTML("beforeend",t),S.refresh()}const i={formEl:document.querySelector(".form"),inputEl:document.querySelector("input"),loaderEl:document.querySelector(".loader"),loadMoreBtn:document.querySelector(".load-btn")};let u,l=1,h=0;const B=15;let n;i.formEl.addEventListener("submit",P);i.loadMoreBtn.addEventListener("click",x);async function P(o){if(o.preventDefault(),y(),p.innerHTML="",u=i.inputEl.value.trim(),u==="")return d(),m.warning({title:"Caution",message:"Please complete the field!",position:"topRight"});l=1;try{if(n=await f(u,l),h=Math.ceil(n.totalHits/B),n.hits.length===0)return d(),b(),m.info({title:"Sorry",message:"There are no images matching your search query. Please try again!",position:"topRight"});g(n.hits)}catch(t){console.error("Error fetching images:",t)}finally{d()}L(),i.inputEl.value=""}async function x(o){y(),l+=1;try{n=await f(u,l),g(n.hits)}catch(t){console.error("Error fetching images:",t)}finally{d(),L(),T()}}function y(){i.loaderEl.classList.add("is-open")}function d(){i.loaderEl.classList.remove("is-open")}function R(){i.loadMoreBtn.classList.add("is-open")}function b(){i.loadMoreBtn.classList.remove("is-open")}function L(){if(l>=h)return b(),m.info({title:"Sorry",message:"We're sorry, but you've reached the end of search results.",position:"topRight"});R()}function T(){const t=p.firstElementChild.getBoundingClientRect().height*2;scrollBy({top:t,behavior:"smooth"})}
